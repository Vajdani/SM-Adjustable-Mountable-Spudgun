...
function Handheld:client_onClientDataUpdate( data, channel )
	self.cl.data = data.data
	self.cl.templates = data.templates
end

function Handheld:server_onCreate()
    self.sv = {}
...
function Handheld.client_onCreate( self )
	self.cl = {}
...
function Handheld:cl_shoot()
    if not sm.game.getEnableAmmoConsumption() or sm.container.canSpend( sm.localPlayer.getInventory(), obj_plantables_potato, 1 ) then
        ...
        if owner then
            for i = 1, self.cl.data.shots do
                local dir = sm.noise.gunSpread( dir, self.cl.data.spread )
                sm.projectile.projectileAttack( projectileTypes[self.cl.data.type].projectile, self.cl.data.damage, firePos, dir * self.cl.data.fireForce, owner, fakePosition, fakePositionSelf ) 
            end
        end
...
function Handheld.client_onRefresh( self )
	self:loadAnimations()
end

function Handheld.loadAnimations( self )
...
function Handheld:cl_projType(button)
...
function Handheld:cl_fullAuto(button)
	if not self.cl.data.controlled then
...
function Handheld:cl_dmg(button)
	if not self.cl.data.controlled then
...
function Handheld:cl_sprd(button)
	if not self.cl.data.controlled then
...
function Handheld:cl_rld(button)
	if not self.cl.data.controlled then
...
function Handheld:cl_proj(button)
	if not self.cl.data.controlled then
...
function Handheld:cl_vel(button)
	if not self.cl.data.controlled then
...
function Handheld:cl_template()
	if templates ~= nil and #templates > 0 and not self.cl.data.isTemplate then
...
function Handheld:cl_value()
	self.cl.valueIndex = self.cl.valueIndex == #values and 1 or self.cl.valueIndex + 1
end

function Handheld:client_onReload()
	self.cl.gui:open()

    return true
end

function Handheld:client_onToggle()
	return true
	...
	--[[if not self.cl.data.controlled and not self.cl.data.isTemplate then
		...
		self.network:sendToServer("cl_save")
...
function Handheld:server_onFixedUpdate( dt )
	--[[if self.sv.data.isTemplate and self.sv.data ~= self.interactable:getPublicData() then
		sm.interactable.setPublicData( self.interactable, { name = tostring(self.gunId), data = self.sv.data } )
	end
...
function Handheld:client_onFixedUpdate( dt )
	if self.cl.data == nil then return end
    self.shootEffect = self.cl.effects[self.cl.data.type][1]
    self.shootEffectFP = self.cl.effects[self.cl.data.type][2]

	if not self.tool:isLocal() or not self.tool:isEquipped() then return end
    if self.cl.data.fullAuto and self.cl.isFiring then
        self.cl.fireCounter = self.cl.fireCounter + dt
        if sm.game.getCurrentTick() >= self.cl.lastShotTick + self.cl.data.delay then
            self.cl.fireCounter = 0
            self:cl_shoot()
        end
    end
...
function Handheld.client_onUpdate( self, dt )
	...
	if self.tool:isLocal() then
		if self.cl.data ~= nil and self.cl.gui:isActive() then
			...
			if not self.cl.data.isTemplate then
				local txt2 = templates ~= nil and #templates > 0 and self.cl.data.templateCount > 0 and templates[self.cl.data.templateCount].name or "none"
...
function Handheld.client_onEquip( self, animate )
...
function Handheld.client_onUnequip( self, animate )
...
function Handheld.sv_n_onAim( self, aiming )
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function Handheld.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim( aiming )
	end
end

function Handheld.onAim( self, aiming )
	self.aiming = aiming
...
function Handheld.sv_n_onShoot( self, dir )
	self.network:sendToClients( "cl_n_onShoot", dir )
end

function Handheld.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function Handheld.onShoot( self, dir )
...
function Handheld.calculateFirePosition( self )
...
function Handheld.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function Handheld.calculateFpMuzzlePos( self )
...
function Handheld.cl_onPrimaryUse( self, state )
...
function Handheld.cl_onSecondaryUse( self, state )
...
function Handheld.client_onEquippedUpdate( self, primaryState, secondaryState )
...
function Handheld:client_onDestroy()
